...
namespace Manimal.QuestBriefingAPI
{
    ...
    public sealed class BriefingPlayer : MonoBehaviour
    {
        ...
        {
            if (_active != null) _active.SuspendPlayback();
        }

        public void Initialize(TMP_Text description)
        {
            _description = description;
            // Isolate source effects from any sounds attached to the vanilla panel.
            ...
            // Separate source keeps the squelch independent of narration filters.
        ...
        internal void Select(string questId, bool unlocked, BriefingRecording recording)
        {
            ...
            _questId = questId;
            _unlocked = unlocked;
            if (!changed) return; // Accept/status redraws must not restart a stopped briefing.
            ResetAudio();
            _selection.Clear();
            if (!_enabled || !unlocked) { ShowControls(false); return; }
        ...
        private void Update()
        {
            ...
            if (_enabled != enabled)
            {
                ...
                ResetAudio();
                _selection.Clear();
                ShowControls(enabled && _questId != null && _unlocked);
                if (enabled && _questId != null && _unlocked)
                {
            ...
            if (_bar == null || !_bar.gameObject.activeSelf || _clip == null) return;
        ...
        {
            if (!Application.isFocused || !isActiveAndEnabled || _description == null
        ...
        private void PauseForForeground()
        {
            ...
            if (_clip != null && _selection.WantsPlayback)
        ...
        private void ApplyRadioFilter(bool force = false)
        {
            ...
            _highPass.enabled = enabled;
            _lowPass.enabled = enabled;
            _distortion.distortionLevel = distortion;
            _distortion.enabled = enabled && distortion > 0;
        }
        ...
        private void LateUpdate()
        {
            ...
            if ((fitter == null || !fitter.enabled || fitter.verticalFit == ContentSizeFitter.FitMode.Unconstrained)
                && (parentLayout == null || !parentLayout.enabled))
            {
        ...
        private IEnumerator LoadClip(string path, int revision)
        {
            ...
            catch (Exception ex) { LoadFailed(ex.Message); yield break; }
            yield return operation;
            if (!_selection.IsCurrent(revision)) { DisposeRequest(); yield break; }
            try
            {
                if (_request.result != UnityWebRequest.Result.Success)
                    throw new IOException(_request.error);
                _clip = DownloadHandlerAudioClip.GetContent(_request);
                if (_clip == null || _clip.length <= 0) throw new IOException("Recording has no decodable audio.");
                _source.clip = _clip;
        ...
        private void Play()
        {
            ...
            _selection.Play();
            if (_clip == null) { LoadRecording(); return; }
            // All starts go through Update's foreground gate, including async loads
            // and manual replay. A selected quest can still be behind a modal.
        }
        ...
        {
            if (_cueSource == null || !UseRadioCues || !BriefingSettings.Enabled.Value) return;
        ...
        private void StopPlayback(bool disconnect)
        {
            ...
            _wasPlaying = false;
            if (_progress != null) _progress.SetValueWithoutNotify(0);
            if (_clip != null) { SetTime(0); _status.text = "Stopped"; }
        }
        ...
        private void ResetAudio()
        {
            ...
            if (_clip != null) Destroy(_clip);
            _clip = null;
            if (_progress != null) _progress.interactable = false;
        }
        ...
        private void OnDestroy()
        {
            ...
            if (_space != null) Destroy(_space);
            if (_audioObject != null) Destroy(_audioObject);
            if (_cueObject != null) Destroy(_cueObject);
            if (_openingCue != null) Destroy(_openingCue);
            if (_closingCue != null) Destroy(_closingCue);
        }
        ...
        private void BuildControls()
        {
            ...
            // Progress occupies y=0..16; controls y=24..56. Explicit anchors keep
            // them separate at every canvas scale (the old rectangles overlapped).
            ...
            // A fill-only slider still supports clicking and dragging, without
            // Unity stretching a handle vertically across the control strip.
            ...
            UnityEngine.Events.UnityAction action)
        {
            ...
            image.color = Color.clear; // Invisible 32px hit area, not a boxed text button.
