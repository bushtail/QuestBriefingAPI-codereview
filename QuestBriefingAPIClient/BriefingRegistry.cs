...
internal sealed class BriefingRegistry
{
        ...
        string traderId = null, bool? radioFilter = null, bool? radioCues = null)
    {
        try
        {
            return Add(Create(ownerId, questId, directory, file, traderId, radioFilter, radioCues));
        }
        catch (Exception ex)
        {
    ...
    internal bool Unregister(string ownerId, string questId)
    {
        if (questId == null || !_recordings.TryGetValue(questId, out var recording)
                            || !string.Equals(ownerId, recording.OwnerId, StringComparison.Ordinal))
        {
            return false;
        }
    ...
    private bool LoadPack(string manifest)
    {
        try
        {
            ...
            using var streamReader = new StreamReader(manifest);
            using var textReader = new StrictTextReader(streamReader);
            ...
            var pack = serializer.Deserialize<Pack>(textReader);
            if (pack == null) throw new InvalidDataException("Manifest cannot be empty.");
            ...
            {
                throw new InvalidDataException("packId is required.");
            }

            if (_packs.Contains(pack.PackId))
            {
                throw new InvalidDataException($"Duplicate packId '{pack.PackId}'.");
            }
            ...
            foreach (var entry in pack.Briefings)
            {
                if (entry == null)
                {
                    throw new InvalidDataException("A briefing entry cannot be null.");
                }

                var recording = Create(pack.PackId, entry.QuestId, directory, entry.File,
                    entry.TraderId ?? pack.TraderId, entry.RadioFilter ?? pack.RadioFilter,
                    entry.RadioCues ?? pack.RadioCues);

                if (!ids.Add(recording.QuestId))
                {
                    throw new InvalidDataException($"Repeated questId '{recording.QuestId}' in pack.");
                }
        ...
        {
            Warning($"[Briefings] Skipping '{manifest}': {ex.Message}"); 
            return false;
        }
    }
        ...
        string traderId, bool? radioFilter, bool? radioCues)
    {
        ...
        if (!IsId(questId))
        {
            throw new ArgumentException("questId must contain 24 hexadecimal characters.");
        }
        if (traderId != null && !IsId(traderId))
        {
            throw new ArgumentException("traderId must contain 24 hexadecimal characters.");
        }
    ...
    {
        if (id is not { Length: 24 }) { return false; }
    ...
    internal static string SafePath(string directory, string file, bool audio = true)
    {
        ...
        {
            throw new ArgumentException("file escapes its pack directory.");
        }
